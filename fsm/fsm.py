...
class BaseFiniteStateMachineMixin:
    ...
    def change_state(self, next_state, **kwargs):
        ...
        entry_fn_name = "on_entry_{0}".format(next_state)
        entry_fn = getattr(self, entry_fn_name, None)
        exit_fn_name = "on_exit_{0}".format(previous_state)
        exit_fn = getattr(self, exit_fn_name, None)
